import { useState } from 'react';
import axios from 'axios';

const FilmReq = () => {
    const [data,setData]= useState("");
    const [apiKey, setApiKey]= useState("");
    const [movieTitle, setMovieTitle]= useState("");

    const updateApiKey = (event) => {
        event.preventDefault();
        setApiKey(event.target.value);
    }
   
    const updateMovieTitle = (event) => {
        event.preventDefault();
        setMovieTitle(event.target.value);
    }
    const makeRequest =(event) => {
        axios.get("http://www.omdbapi.com?apikey=" + apiKey + "&t=" + movieTitle)
        .then(response => {
            console.log(response.data);
            setData(response.data);
        });
};

return (
    <div>
        <div>
            <h2>FilmRequst.js</h2>
            <p>Please enter your API key from OMDB here (b8a47d0)</p>
            <input type="text" onChange={(e) => updateApiKey (e)} />
            <p> Enter the name of the film you are searching for here</p>
            <input type="text" onChange={(e) =>updateMovieTitle(e)}/>
            <p> 
                Please do not click this button until the fields above are correctly filled
            </p>
            <button onClick={(e) => makeRequest(e)}> Click me </button>

            <h4> {data.Title}</h4>
            <h4> {data.Year}</h4>
            <h4> {data.Rated}</h4>
            <h4> {data.Genre}</h4>
            <h4> {data.Plot}</h4>
            <img src={data.Poster} alt="The poster"></img>
        </div>
    </div>
);
}
export default FilmReq